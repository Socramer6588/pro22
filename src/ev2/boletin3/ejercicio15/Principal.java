...
public class Principal {
    ArrayDeque<Aparato> cola = new ArrayDeque<>();

    public static void main(String[] args) {

        Principal obj = new Principal();
        obj.agregarElemento(new Frigorifico(200, 60));
        obj.agregarElemento(new Lavadora(500, 30));
        obj.agregarElemento(new Frigorifico(500, 80, 'E', "negro", 420, 165));
        obj.agregarElemento(new Lavadora(600, 25, 'B', "blanco", 9, true));
        obj.agregarElemento(new Aparato(600, 20, 'D', "gris"));
    ...
    private boolean existeElemento(Aparato a) {
        if (cola.contains(a)) {
            return true;
        }
        return false;
    ...
    private void agregarElemento(Aparato a) {
        if (!existeElemento(a))
            cola.add(a);
        else
            System.out.println("El elemento ya existe en la cola");
    }

    private Aparato extraerElemento() {
        return cola.remove();
    }

    private void listarElementos(){
        Iterator elementos = cola.iterator();
        while (elementos.hasNext()) {
            Aparato a = (Aparato) elementos.next();
